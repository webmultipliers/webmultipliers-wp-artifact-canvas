...
class GitWebhook {
	...
	public function register_routes(): void {
		register_rest_route( 'wmac/v1', '/git-webhook/(?P<id>[\d]+)', [
	...
	public function handle( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		$note = trim( implode( ' @ ', array_filter( [ $ref, $sha ] ) ) );
	...
	private function verify_signature( \WP_REST_Request $request ): true|\WP_Error {
	...
	private function resolve_archive_url( array $payload ): string|\WP_Error {
		// Custom CI: explicit archive URL in payload
		if ( ! empty( $payload['wmac_archive_url'] ) ) {
			return esc_url_raw( (string) $payload['wmac_archive_url'] );
		}
		...
		if ( ! empty( $payload['project']['http_url_to_repo'] ) && ! empty( $payload['ref'] ) ) {
			...
			$ref  = sanitize_text_field( (string) preg_replace( '/^refs\/(?:heads|tags)\//', '', $payload['ref'] ) );
			return "{$repo}/-/archive/{$ref}/archive.zip";
		}
